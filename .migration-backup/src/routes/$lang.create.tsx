...
export const Route = createFileRoute("/$lang/create")({
  head: ({ params }) => {
    try {
      const lang = isLocale(params.lang) ? params.lang : "id";
      return { meta: [{ title: lang === "id" ? "Buat Materi Baru — Lectra" : "Create new lesson — Lectra" }] };
    } catch {
      return { meta: [{ title: "Lectra" }] };
    }
  },
...
function CreatePage() {
  const { lang } = useParams({ from: "/$lang/create" });
  ...
  async function onSubmitTopic(e: React.FormEvent) {
    ...
    try {
      ...
      navigate({ to: "/$lang/lesson/$id", params: { lang: locale, id: saved.id } });
    } catch (err) {
    ...
    <div className="min-h-screen bg-background">
      <header className="px-6 md:px-12 py-6">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link to="/$lang" params={{ lang: locale }} className="inline-flex items-center gap-2 text-sm font-medium hover:opacity-70 transition-opacity">
        ...
        <div className="max-w-3xl mx-auto">
          ...
          >
            <h1 className="font-display text-4xl md:text-6xl leading-tight mb-4">
              {t.heading1}<em className="not-italic text-accent">{t.headingEm}</em>?
            </h1>
            ...
            {(["topic", "photo"] as CreateMode[]).map((m) => (
              <motion.button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition ${
                  mode === m ? "bg-ink text-background shadow-md" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {photos.map((f, i) => (
                      <div key={i} className="relative w-16 h-16 rounded-xl overflow-hidden border border-border">
                        ...
                        <button type="button" onClick={() => removePhoto(i)} className="absolute top-0.5 right-0.5 w-5 h-5 bg-ink text-background rounded-full grid place-items-center">
                          <X className="w-3 h-3" />
                        </button>
                      </div>
                    ))}
              ...
              {t.suggestions.map((label, i) => {
                ...
                return (
                  <motion.button
                    key={label}
                    type="button"
                    className="suggestion-chip group bg-card border border-border rounded-2xl p-4 text-left hover:border-ink transition disabled:opacity-50 opacity-0"
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-12">
              ...
              <div className="space-y-2">
                {history.slice(0, 5).map((item) => (
                  <div key={item.id} className="group flex items-center justify-between bg-card border border-border rounded-2xl px-4 py-3 hover:border-ink transition">
                    ...
                    <div className="flex items-center gap-1 shrink-0 ml-3">
                      <button type="button" onClick={(e) => handleDeleteHistory(item.id, e)} className="p-1.5 rounded-lg text-muted-foreground hover:text-destructive opacity-0 group-hover:opacity-100 transition">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                      <Link to="/$lang/lesson/$id" params={{ lang: locale, id: item.id }} className="p-1.5 rounded-lg hover:bg-muted transition">
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap gap-2">
        {levels.map((l) => (
          <button key={l} type="button" onClick={() => setLevel(l)} disabled={disabled}
            className={`px-4 py-2 rounded-full text-sm font-medium border transition ${level === l ? "bg-ink text-background border-ink" : "bg-background border-border hover:border-ink"}`}>
            {l}
          </button>
        ))}
      </div>
...
function Alert({ error }: { error: string }) {
  return <div role="alert" className="mt-5 px-4 py-3 rounded-xl bg-destructive/10 text-destructive text-sm">{error}</div>;
}
  ...
  return (
    <>
      <motion.button
        ...
        className="btn-orange w-full justify-center mt-6 hover:btn-orange-hover disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {loading ? <><Loader2 className="w-4 h-4 animate-spin" />{loadingLabel}</> : <>{label} <ArrowRight className="w-4 h-4" /></>}
      </motion.button>
      {loading && (
        <div className="mt-3 h-1 rounded-full bg-border overflow-hidden">
          <motion.div className="h-full bg-accent rounded-full" animate={{ width: `${((step + 1) / steps) * 100}%` }} transition={{ duration: 0.5 }} />
        </div>
      )}
    </>
  );
